...
Class TractorBeam Extends Weapon
	...
	Function RockSuckedUpCallBack:Void(shape:cpShape, pointset:cpContactPointSet Ptr, p:Void Ptr)
		...
		Else
			v = cpv(-normal.x, -normal.y)
		End If
		'Get the current velocity of the rock object
		Local cv:=cpBodyGetVelocity(r.Body)
		...
		'This is onle of the main things that makes this work ok. We cancel gravity on the rock object when it is touched by the beam
		...
		cpShapeGetBody(shape).Angle = ATan2(player.Facing.x, player.Facing.y)
